...
後続で[パラフレア・オフセット影・オフセットリムの12スライダーをキー化](./depth-effects-timeline-2026-09-14.md)した。以下は着手前の候補整理。

所有者の追加指定により、今後のキー対応は現在UIから選択できる項目に限定する。内部IDや旧設計名だけから候補を追加しない。没・非表示の効果を復活させる作業は含まない。

- オフセット影／オフセットリム: 数値uniformをキーへ接続し、初期OFF時の深度準備を今回と同様に固定する方針が候補。通常パネルの全スライダーとClassic側の実装差を確認してから追加する。
- パラフレア: 現在UIで選べる強度・グラデーション偏りを候補とする。内部IDはdirectionalLightShaftsだが、旧光芒のサンプル処理は廃止済み。旧光芒をキー化・復活する候補ではない（[方針転換と現行実装](./framegraph-directional-light-shafts-implementation-2026-08-12.md)）。
