...
- 背景メニューに「標準背景」「白背景」「黒背景」「透明チェック背景（プレビュー）」を追加する。
- 4種類は同時に1つだけ選択できるため、メニュー上ではラジオ項目として表示する。
- 4種類の背景表示を排他的に切り替え、`viewport.backgroundDisplayMode` に保存する。
...
- 背景表示モードの初期値は `default`。既存のスカイドーム、背景画像、背景動画を表示する。
