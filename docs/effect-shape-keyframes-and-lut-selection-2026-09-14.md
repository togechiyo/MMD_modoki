...
後続で[空気遠近の3スライダーをキー化](./aerial-perspective-timeline-2026-09-14.md)した。以下の一覧は本メモ作成時点の残件。所有者の追加指定により、今後は現在UIから選べる項目だけを対象にする。旧「光芒」は廃止され、同じ内部IDの現行UIは「パラフレア」であるため、候補名を訂正した。

対応済み10効果の通常パネルのスライダーは今回で対象に入る。色picker、非表示の光条等は今回追加しない。海を除く残り10効果は引き続き別作業。

- 空気遠近・パラフレア・オフセット影／オフセットリム: 数値補間に加え、深度の事前確保とOFF時の中立化を接続する。
