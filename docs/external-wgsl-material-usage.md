...
3. 外部WGSL欄の「読込…」で `effect.modoki.json` を選ぶ。WGSL本文は定義の `sources` から読む。
4. 材質行を選んで「選択へ適用」、または「全材質へ適用」。読込だけでは描画を変更しない。
5. パラメーターは数値をEnterで確定する。Escapeまたは未確定のままフォーカスを外すと元に戻る。範囲が指定されたscalarにはスライダーがあり、操作完了時に適用する。

色はカラーピッカーとHEXの表示を基本とし、「数値で編集」を開くとRGB（vec4ならαも）を個別に編集できる。HEXは8bit表示だが、表示しただけで保存値を丸めない。成分を数値編集した場合も、変更していない成分の精度を保つ。αはカラーピッカー操作で変更しない。

適用中の名前と状態は分けて表示し、同じシェーダーの名前を重複させない。別の定義を読んだ場合だけ、適用待ちの候補を「読込済み」として併記する。上段の読込・再読込・解除と、下段の適用先ボタンを分けている。
...
「再読込」は選択材質の元ファイルを読み直して適用する。初期実装では再読込時のパラメーターは新定義の既定値に戻る。失敗時は前の割当・値を維持する。「解除」は選択材質の外部処理を取り除く。適用、再読込、解除、パラメーター変更はUndo/Redo対象。
