...
### 2026-08-27 3色グラデーション試作の撤回

遠景の層ごとに色温度を変える3色グラデーションと中間位置調整を試作したが、距離と色の関係が把握しづらく、通常の空気遠近設定としては操作項目が過剰になった。所有者判断により試作を撤回し、色指定は従来の単色へ戻した。霞の濃さは引き続き開始距離と遷移距離から解析的に求め、段差、noise、追加passは導入しない。
