...
カメラpayloadの `distances[0]` はsource animation / VMD互換の正本値であり、通常は負の距離として保持される。2026-09-16、V022-070対応で補正時の加算方向を変更した。補正とpreviewでは `編集距離 = -source距離` として扱い、式を適用してからsourceの符号へ戻す。

例えばsource `-45`（通常UIの距離45）に `+5` を適用するとsource `-50`（距離50）となり、遠ざかる。`-5` ならsource `-40`となり近づく。倍率は従来どおりで、保存形式は変更しない。共通helperを使うGUI / Action / MCPへ同じ規約を適用する。

絶対値化やクランプは行わない。正のsource距離は補正座標上の負の距離として保持し、恒等補正でも向きを反転させない。ゼロをまたぐ加算と負の倍率も符号付きのまま計算する。例えばsource `-45` に `-50` を加算するとsource `+5`になる。

修正前に回帰unit 8件の失敗を確認し、修正後はfocused unit 17件成功。Windows / WebGPUのGUI E2Eで2キーへの距離 `+5`、位置・回転との同時補正、Undo / Redo、project再読込後の距離表示35を確認した。全unit 888件、lint成功。元報告のM4 Mac環境は未確認。
...
- カメラ距離の加算方向は2026-09-16に修正。元M4 Mac環境での操作感は再確認待ち。
