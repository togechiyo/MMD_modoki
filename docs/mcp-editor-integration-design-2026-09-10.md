...
後続の明示条件: MCPからアプリ操作を可能にし、ファイルパスとキーフレーム情報は共有してよいが、モデルそのものは送信しない。モデル本体・テクスチャ原本・形状データを返すtoolやResource、汎用file読込を公開しない。viewportの表示画像と編集に必要なメタデータは区別する。以下にある将来のfile操作・export案もこの制約の範囲内で再設計する。

さらに所有者は、主用途のキーフレーム編集に加えて、UIから操作できる項目を一通りMCPから操作できることを希望した。初期tool群だけで完了とせず、[実装状況のUI対応表](./mcp-implementation-progress-2026-09-10.md)を使って入力・完了通知・保存値・GUI同期を順次接続する。
...
loopback専用、認証必須、Host/Origin検証は維持する。portは初回に確定した値を設定として保持する。2026-09-18の所有者指定により、保存済みportが使用中（EADDRINUSE）なら空きportを一度だけ自動取得し、資格情報を維持して新しいportを保存する。変更時は設定画面に旧・新portとクライアント側URLの更新案内を表示する。競合以外の失敗や再取得・保存の失敗ではON表示へ進めない。詳細は[ポート競合時の自動復旧](./mcp-port-recovery-2026-09-18.md)を参照する。
...
設計作成時点ではruntime実装・MCP接続・画像取得E2Eは未実施。後続の実装状況は[接続基盤の着手メモ](./mcp-implementation-progress-2026-09-10.md)を参照する。
