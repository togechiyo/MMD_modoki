...
保存済みportが他のアプリに使われている場合は、空きportを自動取得・保存する（2026-09-18追加）。MCP設定画面に変更案内が出るので、「接続設定を表示」で新しいURLを確認し、クライアント側のURLを更新する。認証トークンは変わらない。[仕様・検証](./mcp-port-recovery-2026-09-18.md)。

`mmd_capture_viewport_sequence` で数秒分の画像をまとめて取得できる（2026-09-18追加）。既定3秒×2fps・長辺640px・JPEG。参照のみでも再生中の撮影が可能。撮影自体は再生/停止/seekを行わない。実時刻と欠落枚数を読み、固定fpsの動画として扱わない。[方式判断・契約・検証](./mcp-viewport-sequence-2026-09-18.md)。
...
この表は完成率の見積りではなく、既存UIの分野別棚卸し。2026-09-11の[公開ツール・エフェクト・材質追加](./mcp-public-ui-tools-effects-2026-09-11.md)まで更新。非公開・没機能は所有者指定により追加対象外。AI向けにも `mmd_help` の `ui-coverage` / `keyframes` / `ui-settings` / `materials` / `files-and-output` / `file-tools` / `material-batch` を公開し、本文を含めて検索できる。

| UI分野 | 接続済み | 未接続の主な項目 |
| --- | --- | --- |
| 再生・時間移動 | play/pause/seek、loop、再生範囲 | 隣接キー移動の専用入口 |
| 編集対象 | モデル/カメラ/アクセサリ選択、複数ボーン選択 | モーフ選択 |
| ポーズ・表情 | カメラ/単一・複数ボーン/モーフpreview、現在値登録、ボーン/モーフのキー値設定、複数IKのON/OFF | world座標や目標からの自動ポーズ生成 |
| キー編集 | 値/補間参照、set/delete/copy/move、現在値登録、ミラー、列挿入/削除、補正、batch、GUI/MCP Undo/Redo、自動キー設定、GUI範囲選択・共通クリップボード・移動削除、体格補正 | 複数モデルをまたぐ一括編集 |
| シーンキー | カメラ、照明、影、重力、アクセサリ、モデル表示/IKのキー値、モデル/カメラ外部親の一括編集・dryRun、アクセサリ親/変形 | 複数モデル間の外部親同時組替え |
| 材質 | モデル/アクセサリの表示・内蔵preset・resetの一括適用、通常/PBR切替、追加許可による詳細参照 | 非公開の物性値編集は追加対象外 |
| 表示・実行 | 地面、空、背景メディア、AA、物理、影、剛体/物理ボーン表示、床衝突、環境、エッジ、物理評価buffer/全減衰補正 | 公開設定の残る個別検証 |
| 色・描画 | 基本色調整、Bloom/DOF/SSAO/SSR/Fog、照明・影、SSGI/Luminous/空気遠近/光芒/オフセット影・光/リング粒子、DOF対象/LUT、Frame Graph効果順序（検索カタログ97項目） | 描画/物理backendのreload切替等。非公開水面などは対象外 |
| ファイル・素材管理 | 元path一覧、明示path読込、モデル/アクセサリ・音声・背景・カメラモーション・外部環境/LUTの指定削除 | 統合済みモデルモーションの個別削除、差替え |
| 保存・出力 | project保存/復元、VMD/VPD/BVMD、PNG、出力条件、別ウィンドウWebM・PNG連番と進捗・取消 | 別プロセス単発PNG出力 |
| ツール | PMX/PMD→BPMX、VMD→BVMD、PMXを参照するVMDリターゲットの一括変換・項目別結果・取消 | 変換中の単一項目の強制中断 |
