...
全UI対応は未完了。以下を残件として扱う。

1. PNG連番・別プロセスPNG出力。WebM動画・進捗・取消は [2026-09-11追加](./mcp-video-and-asset-removal-2026-09-11.md) で接続。
2. 素材の差替え、統合済みモデルモーションの個別削除、モデル全体表示、アクセサリのpreview変形・表示・親。モデル/アクセサリ・カメラモーション・音声・背景・外部環境・LUTの指定削除は2026-09-11追加で接続。
3. 専用IK編集、一括ポーズpreview、GUIキー範囲選択・クリップボード、体格補正。モデル/カメラ外部親の取得・一括編集・dryRunは [2026-09-11追加](./mcp-external-parent-2026-09-11.md) で接続。
