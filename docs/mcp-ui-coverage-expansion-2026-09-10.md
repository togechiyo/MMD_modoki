...
1. PNG連番・動画・別プロセス出力と取消。既存exporterの受付・進捗・完了・取消をjobへ接続し、ウィンドウを開いただけで成功としない。
2. 素材の削除・差替え、モデル全体表示、アクセサリのpreview変形・表示・親。現在の選択/外部親依存/履歴との整合を確認する。
3. 専用IK/外部親編集、一括ポーズpreview、GUIキー範囲選択・クリップボード、体格補正。
