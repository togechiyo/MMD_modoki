...
- [ ] アクセサリ欄の既存操作と保存同期を棚卸し
- [ ] アクセサリ専用欄を廃止し、Camera / Model / Accessory を情報欄の対象一覧へ統合
- [ ] 情報欄、下パネル、タイムラインの対象選択同期を整理
- [ ] 旧アクセサリ欄を撤去し、空いた下パネルへ重力欄の UI 枠を配置
- [ ] 未接続のシーンキー操作を無効表示または準備中表示にする
- [ ] タイムライン仕様を現行実装へ更新
- [ ] `modoki-owned tracks` の共通型、評価、project 保存、CommandDiff の責務を設計
- [ ] 照明の色 RGB / 方向 XYZ キーを登録・再生・シーク・保存・undo / redo 対応
- [ ] タイムライン操作を選択キー操作 / 時間軸構造操作 / 値補正へ整理
- [ ] 空フレーム挿入 / フレーム列削除を batch key edit として実装
- [ ] 選択キーの位置 XYZ 補正を1操作単位で実装
- [ ] カメラ距離 / FoV 補正を実装
- [ ] 回転 XYZ 補正の Euler / Quaternion 整合を調査
- [ ] セルフ影のモード / 距離キーを実装
- [ ] 重力キーのシーク再現性を調査し、方向 / 加速度 / ノイズを project 独自トラックとして実装
