...
- [x] アクティブモデル切り替え
- [x] VMD モーション読み込み
- [x] 同名boneの回転専用 / 移動付きtrackを統合し、非表示boneを含む全motion削除と個体別Undo / Redo・保存再読込を確認（[V022-075修正](./issue-25-project-motion-retention-2026-09-12.md#2026-09-12-後続修正)）
- [x] VPD ポーズ読み込み
- [x] カメラVMD 読み込み
...
- [x] [選択ボーンを注視](./camera-focus-selected-bones-2026-09-15.md): 表示メニュー/Fキー/MCPから注視点を合わせ、複数選択・Undo/Redo・外部親カメラを扱う。
...
- [x] 背景画像インポート
- [x] 背景動画インポート
- [x] 背景メディアの表示状態を保存・出力へ復元し、effect用描画先への登録と動画シーク完了待機を修正。Windows fixtureで確認、元M4 Macは再確認待ち（[Issue #26修正](./issue-26-background-media-output-2026-09-16.md)）
...
- [x] BVMD 3.0 書き出し（編集元モデル / カメラmotionを分離し、UTF-8 track名を保持）
- [x] VPD ポーズ書き出し（β実装・MMD 本家での基本互換確認済み）
- [x] プロジェクト保存 / 読み込み（JSON）
- [x] プロジェクトへキーフレーム本体を保存 / 復元
- [x] 新規プロジェクトを別ウィンドウで開き、元プロジェクトの状態・履歴・保存先を保持
- [x] プロジェクトのD&Dは初期状態の空ウィンドウへ読み込み、編集中・読み込み済みなら別ウィンドウで開く（2026-09-18、[Action仕様](./actions/project-actions.md)）
...
- 2026-09-02: ファイルメニューへモデル / カメラBVMD 3.0書き出しを追加。編集元`MmdAnimation`から直接変換するためVMD / Shift_JISを経由せず、中国語系漢字・異体字・長いtrack名をUTF-8のまま保持する。モデルとカメラは別fileとし、BVMD 3.0に領域がない外部親keyは警告して除外する。Unicode round-trip unit testとGUIの登録・保存E2Eを確認済み。
...
- [x] アクセサリの表示ON/OFFを位置・回転・倍率と同じキーへ保存し、X / OBJ / 既存GLB復元経路の編集・コピー・Undo・project往復に接続（2026-09-17、上記仕様）。GLB描画は従来の無効化範囲として別管理。
...
- [x] v0.2.4の版情報・README・リリースノート案・既知課題・文書索引を整備（[公開前確認と残件](./v0.2.4-release-preflight-2026-09-18.md)。タグ・配布ビルド・公開は未実施）
...
- [x] 独自SSSのダンス時のぶれとSkin / Waxの陰影・血色を調整し、所有者が最終ルックを採用（2026-09-07実機OK、[最終設定・検証範囲](./owned-sss-development-2026-09-06.md)。全モデル・全性能条件の保証とはしない）
- [x] 背景向け汎用マット材質 `Stage Standard` の初期実装とローカル描画確認（[仕様・検証範囲](./stage-standard-material-2026-09-07.md)。所有者の実ステージ評価は別途）
...
- [x] [パッケージ版MCP実用テストで判明した問題](./mcp-packaged-practical-test-2026-09-11.md)を修正・再検証する。WebM出力、モデルコメントのユーザー待ち通知、snapshot、照明色、復旧案内・カメラVMD警告を修正。配布exe E2EとAliciaの通常/PBR動画を確認（[修正記録](./mcp-practical-fixes-2026-09-11.md)）。

- [ ] MCPによるAI連携と実験設定内のON/OFFを追加する。主用途のキー編集を優先しつつ、公開UIから操作できる項目を一通りMCPへ接続する。非公開・没機能は対象外。モデル本体は送信しない。接続基盤、viewport画像・元path・help、カメラ/ボーン/モーフpreview、現在値登録、キー値/補間参照・一括set/delete/move/copy、ミラー・フレーム列挿入/削除・値補正・Undo/Redo、対象切替、設定15項目、モデル材質表示まで接続。構造化エラー・復旧案内、busy理由、直近失敗と実行状態の診断取得も追加。説明付き追加許可で単一対象のボーン・モーフ・材質・剛体・ジョイント詳細を取得し、提供履歴をUI表示。さらに設定カタログ97項目、複数ボーン選択、自動キー/再生範囲/出力条件/言語/UI倍率/全画面、モデル/アクセサリ材質preset、明示path素材読込・project保存/復元・PNG/VMD/VPD/BVMD出力、通常/PBR切替とjob結果照会を接続。さらに[PNG連番の進捗/完了/取消](./mcp-png-sequence-2026-09-11.md)と[WebM動画の進捗/完了/取消と素材指定削除](./mcp-video-and-asset-removal-2026-09-11.md)を追加。全UI対応は未完了で、[拡張と残件](./mcp-ui-coverage-expansion-2026-09-10.md)にある別プロセス単発PNG・統合モーション個別削除/差替え等と個別E2Eを続ける（[実装状況・UI対応表](./mcp-implementation-progress-2026-09-10.md)、[操作・情報取得・ヘルプ設計](./mcp-editor-integration-design-2026-09-10.md)、[方式比較](./mcp-integration-design-investigation-2026-09-10.md)） [キー範囲選択・GUI共通クリップボード・移動/削除](./mcp-key-selection-2026-09-11.md)と、[複数ボーンの一括ポーズpreview・dryRun・共有Undo](./mcp-pose-editing-2026-09-11.md)、モデル表示・複数IK・アクセサリ親/変形・物理キー入力も追加（[モデル・アクセサリ編集](./mcp-object-editing-2026-09-11.md)）。モデル/カメラ外部親の一括登録・解除・dryRun・循環検証・Undo/Redoも追加（[外部親編集](./mcp-external-parent-2026-09-11.md)）。[公開ツール・エフェクト・材質](./mcp-public-ui-tools-effects-2026-09-11.md)の一括変換、体格補正/Undo、材質表示/preset/resetの一括操作、公開効果41設定も追加。 [キー検索・一括モーフ・描画待機と画像比較](./mcp-search-expression-comparison-2026-09-11.md)も追加。
- [x] [MCPメニューバー対応](./mcp-menu-bar-coverage-2026-09-15.md): 公開87項目の対応一覧、不足していた視点・前後キー・カテゴリ全選択・全モーション削除/Undo・描画順・19設定・新規ウィンドウを追加。Classic / Frame GraphでGUI E2E確認。物理runtime/backend切替等の残件は同メモを参照。
- [x] [MCPのポート競合時の自動復旧](./mcp-port-recovery-2026-09-18.md): 保存済みポートが使用中なら空きポートを再取得・保存し、クライアント側URLの更新を案内する。
...
- [ ] 既知バグ一覧の整備
- [ ] 実験機能フラグ管理
- [x] 設定メニューの実験設定にPBRの明示的ON/OFF、環境ライト詳細、ログ操作を集約（IBL影は凍結理由の表示のみ、[実装メモ](./experimental-settings-popup-2026-09-08.md)）
- [x] 実験設定のMCP固定文言と環境ライトの翻訳漏れを修正し、5言語の説明・状態・ポート変更案内をGUI確認（2026-09-18、[検証](./experimental-settings-popup-2026-09-08.md)）
- [x] タイムラインのカメラ・照明・影・重力の表示名をUI言語へ同期し、下部パネルと用語を統一（2026-09-18、[仕様・検証](./timeline-spec.md)）
...
- [x] 複数モーフ登録時の顔消失（V022-087）。最低8・登録済みtarget数＋4枠を確保し、未登録previewの不足時は自動拡張。アリシア、標準/PBR、project復元・runtime往復切替後のgroup/UV編集を確認。報告者環境は再確認待ち → [調査メモ](./multiple-morph-registration-investigation-2026-09-17.md)
...
- [ ] 外部親登録
  ...
  - [x] カメラ外部親の最小登録UI公開 → [MMD / babylon-mmd 調査・実装 2026-08-10](./camera-external-parent-mmd-babylon-research-2026-08-10.md)
    - [x] 2026-09-16: 初期化前の親行列によるPNG / WebM暗転、旧形式poseのゼロ化、次projectへの親キー残留を修正。Windows E2E確認、元Macは再確認待ち。 → [修正記録](./issue-26-camera-external-parent-output-2026-09-16.md)
...
- [x] 2026-09-14 所有者が海を没と判断。品質改善待ちから却下へ変更し、全エフェクトのタイムライン化対象から除外する。旧実装・project保存互換の削除は今回行わない。
