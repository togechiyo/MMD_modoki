...
- [ ] MCPによるAI連携と実験設定内のON/OFFを追加する。主用途のキー編集を優先しつつ、UIから操作できる項目を一通りMCPへ接続する。モデル本体は送信しない。接続基盤、viewport画像・元path・help、カメラ/ボーン/モーフpreview、現在値登録、キー値/補間参照・一括set/delete/move/copy、ミラー・フレーム列挿入/削除・値補正・Undo/Redo、対象切替、設定15項目、モデル材質表示まで接続。構造化エラー・復旧案内、busy理由、直近失敗と実行状態の診断取得も追加。説明付き追加許可で単一対象のボーン・モーフ・材質・剛体・ジョイント詳細を取得し、提供履歴をUI表示。さらに設定カタログ54項目、複数ボーン選択、自動キー/再生範囲/出力条件/言語/UI倍率/全画面、モデル/アクセサリ材質preset、明示path素材読込・project保存/復元・PNG/VMD/VPD/BVMD出力、通常/PBR切替とjob結果照会を接続。さらに[WebM動画の進捗/完了/取消と素材指定削除](./mcp-video-and-asset-removal-2026-09-11.md)を追加。全UI対応は未完了で、[拡張と残件](./mcp-ui-coverage-expansion-2026-09-10.md)にあるPNG連番・統合モーション個別削除/差替え・外部親等と個別E2Eを続ける（[実装状況・UI対応表](./mcp-implementation-progress-2026-09-10.md)、[操作・情報取得・ヘルプ設計](./mcp-editor-integration-design-2026-09-10.md)、[方式比較](./mcp-integration-design-investigation-2026-09-10.md)） モデル/カメラ外部親の一括登録・解除・dryRun・循環検証・Undo/Redoも追加（[外部親編集](./mcp-external-parent-2026-09-11.md)）。
