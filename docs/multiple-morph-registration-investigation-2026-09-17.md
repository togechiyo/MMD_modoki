...
2026-09-17、所有者から「モーフを複数登録すると顔面が溶けて消えることがある」と報告された（V022-087）。初回はモデル等が未特定。続報でlocal-referencesのアリシアと「あ」「口角上げ」が指定され、当該モデルの利用許可を受けた。元の症状は未再現で、修正済みとは扱わない。
...
これらは再現操作・観測の成功であり、報告された不具合の修正成功ではない。既存project、読み込んだmotion、追加effect、各モーフの値や操作順、使用版を含めた報告時の条件との差は残る。
...
- 報告時の使用版、PBRのON/OFF、材質presetと追加effect、読み込んだmotionやprojectの有無。
- 「あ」「口角上げ」の値、登録順序、個別登録かモーフ欄の一括登録か。
