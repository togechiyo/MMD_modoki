...
採用後の負荷軽減は[独自SSSの性能比較](./owned-sss-performance-2026-09-07.md)を参照。最終ルックのパラメータは維持する。
...
## 2026-09-18: 強い逆光での透過成分を半分にする

所有者から、照明RGB255・強度200で肌が光りすぎるとの報告があった。比較画像は照明・Bloomを維持してSSSからPBR MMD Likeへ材質だけを変更したものと確認。所有者の「逆光での透過を半分くらい」に従い、共通`ownedSssTransmission`の係数を0.75から0.375へ変更する。

対象は自前SSSのSkin / Wax（通常MMD / PBR共通）。肌全体の明るさではなく、厚みと逆光角度に依存する透過光だけを半分にする。拡散半径0.20、厚み減衰距離0.5、陰側の色、受光角度補正、照明120相当の倍率、合成割合、Bloomやシーンの光・影設定は維持する。半減後も照明に比例して増えるため、強い照明での白飛びを完全に防ぐ上限処理ではない。
