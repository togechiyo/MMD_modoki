...
- `物理互換補正` の重力補正は、FollowBone直下の実質自由Y軸＋線形バネで、移動・回転減衰がともに `1.0` 相当のdynamic剛体だけを対象にする。
- `重力補正量 = 0.00` は実効重力 `100%`、`1.00` は `0%`（中心力による相殺）へ線形変換する。
- スライダー既定値は減衰 `0.00`、重力 `1.00`、質量 `0.00`。質量 `0.00` では異常質量補正も適用しない。
- 互換補正はモデル差があるため既定OFFとする。モデル名による個別設定は行わない。動的外部親後の追加剛体同期では補正力を二重注入しない。
- 自由軸バネ重力補正は、MMD本体の物理統合を完全再現するものではなく、反重力中心力による暫定対処とする。比較検証で誤検出が見つかった場合は構造条件を見直す。
