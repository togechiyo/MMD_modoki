...
GUI確認では、ツールバーの言語選択から5モードへ実際に切り替え、少なくともメニューバー、ツールバー、タイムライン、下部パネルを確認する。UI文言やdialogを変更したreleaseでは、変更したdialog、設定画面、出力画面、toast / error表示も対象に含める。各画面で訳語の意味、MMD / 3D編集用語の一貫性、選択locale以外の文言混在も確認する。

次の状態を残したままtagを作らない。

- 翻訳キーがそのまま表示される
- 英語fallbackによって、対象言語の翻訳漏れが隠れている
- 文字化け、空欄、明らかな誤訳、不自然または画面間で矛盾する用語がある
- 文言がlabel領域からはみ出す、切れる、重なる、過度に省略される、操作対象を隠すなど、利用を妨げるlayout崩れがある

軽微な自然さの改善などreleaseを止めない課題は、確認した言語と画面を添えてknown issueまたはfeedback ledgerへ残す。一方、広範な意味誤り、用語不一致、label overflowは単なる表現上の好みとして扱わない。release preflightには、辞書構造、訳語レビュー、GUI layout確認を分けて、確認済み・未確認・既知課題を記録する。
