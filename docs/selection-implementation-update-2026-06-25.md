...
> **位置づけ（2026-08-23更新）:** これは2026-06-25時点の実装履歴である。現在の行・列見出し選択、修飾キー、ダブルクリック変換の正本は [タイムライン仕様](./timeline-spec.md#5-2-選択) を参照する。下記の「トラック名ダブルクリック」「Frameルーラーダブルクリック」「タイムライン Shift + 行名クリック」は現行操作に置き換えられている。
...
- 行名とFrame見出しは、実キーを直接選ぶ場所ではなく「見出し選択」を作る場所になった。
- 通常クリックは単一、`Shift` は同じ軸の連続範囲、`Ctrl` / `Cmd` は個別追加・解除とする。
- 行と列は同時保持せず、最後に操作した軸へ切り替える。行列の和集合は矩形キー選択で作る。
- 行名またはFrame見出しのダブルクリックで、選択中の見出し範囲を実キー選択へ変換する。
- 左上セルはクリックで全解除、ダブルクリックで表示中の全キー選択とする。
- 見出し選択、実キー選択、viewportの複数ボーン選択は別状態である。
- 選択行の高さは変えず、全トラックを均一行高に保つ。
