...
VMD には公開された公式仕様書がない。自動テストと babylon-mmd の読み戻しに加え、2026-08-14にユーザー実機のMMD本家でモデル / カメラVMDの基本読み込み成功を確認した。ただし、境界名、物理ON/OFF、全補間を網羅した互換試験ではないため、機能名には `(β)` を付け、「MMD 完全互換」とは表現しない。
...
## 14. MMD本家での確認状況とβ解除までの残作業

2026-08-14、ユーザー実機のMMD本家で、MMD_modokiが書き出したモデルVMDとカメラVMDの基本読み込み成功を確認した。MMDの詳細versionとfixtureごとの結果は未記録のため、これはリリース前の基本互換確認として扱う。

β解除前に、次のfixtureを個別に記録しながら読み戻す。
