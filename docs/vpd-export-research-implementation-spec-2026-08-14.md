...
機能は β 扱いとする。babylon-mmd の reader / loader と自動テストに加え、2026-08-14 にユーザー実機の MMD 本家で書き出し VPD の読み込み成功を確認した。
...
- [x] MMD 本家で、MMD_modoki が書き出した VPD を読み戻す（2026-08-14 ユーザー実機で成功）
...
基本的な MMD 本家互換は確認できたため実装項目は完了扱いとする。上記の境界条件確認と、βとしてのユーザーフィードバック収集は継続する。
