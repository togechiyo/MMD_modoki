...
| adopted | [上級者向け外部WGSLはMMEの名前と用途を参考にする](./decisions/external-wgsl-follows-mme-concepts.md) | 単一WGSLへ統一し旧JSON読込は撤去。短い名前で既存一覧・割当ボタンへ統合し、追加UIは読込1ボタン。数値はテキスト編集、エラーはviewport通知 |
