...
source_docs:
  - ../../docs/external-hdri-environment-lighting-2026-07-21.md
  ...
  - ../../docs/mcp-menu-bar-coverage-2026-09-15.md
superseded_by: null
...
2026-09-15、所有者は「ツール」にある開発者向け機能の設定パネルを「ウィンドウ」へ移動するよう指定し、続けてメニュー名を「設定」へ変更するよう指定した。現在の入口は「設定 → 実験設定…」とする。既存参照を保つためcardのIDは維持する。

当初は「ツール → 実験設定…」を入口とし、PBRモードに環境ライト・IBL影の詳細をまとめ、ログの場所・コピー操作を置いた。当初は外部WGSLなど他機能を含めず、後続の指示で追加した。
...
2026-09-13、所有者は大項目がOFFのとき、小項目をグレーアウトするかチェックを外すよう指定した。親機能が無効のまま配下の設定を操作可能にしない。実装ではPBRの値を保持して無効表示とし、MCP詳細診断の了承解除は従来の挙動を維持する。

同日の後続了承により、有効化チェックをPBR・WGSL・MCPの大見出しに統合する。見出しの文字もクリック可能にし、別行の有効化チェックを重複させない。
...
同日の追加確認「モデル本体は渡さないけど、ボーンやモーフや材質は何があるか一覧とれないと困る」により、ボーン・モーフ・材質の名前一覧は明示的な公開対象とする。モデル本体を送信しない条件を、編集対象の発見に必要なメタデータまで隠す理由にしない。名前一覧の許可を頂点・モーフ形状差分・モデル構造全体の送信へ広げない。
...
2026-09-17には、所有者が昼・夜のTrueHDRIを用意し、「雪原と同じくサイズ小さくしてこの3種をプリセットとして同梱」と希望した。同梱対象はYamagataField_20181231_1137、EitaiBridge_20190111_1215、MifuneBridge_20190311_2140の縮小版とする。原本16Kの同梱や、将来のHDR素材の無制限な追加を意味しない。
...
2026-09-15、所有者は「メニューバーにある項目ひととおりMCPからもそうさしたい」と指定した。公開UI対応の具体的な確認範囲としてメニューバーを扱い、項目ごとの操作入口と未対応部分を明示する。APIの分割や手動操作として残す項目は[実装メモ](../../docs/mcp-menu-bar-coverage-2026-09-15.md)に記録し、それらの個別制限まで所有者が承認したとは扱わない。
