...
source_docs:
  - ../../docs/sss-strong-backlight-2026-09-18.md
...
2026-09-18、最大照明での過剰な発光感を受け、所有者が逆光の透過を約半分へ抑えるよう指定した。自前SSS共通の透過係数を0.75から0.375へ変更した。続いて、耳・指先など薄い部分の透過と強い輪郭ハイライトを残し、顔全面が発光しない絵作りを希望し、AliciaをRGB255・強度200で検証するよう指定した。透過を切っても残る陰側の底上げとIBLを切り分けた経緯は調査記録を参照する。陰側補正後のルックの採否は所有者確認待ち。

追加実機画像で明暗差の弱さが残り、所有者は陰側の一律な底上げを減らし、薄部透過・輪郭光を際立たせる処理の試用を指示した。Skinの陰色を背面方向へ滑らかに半減する調整を試す。ルックの最終採用を意味する指示ではない。
