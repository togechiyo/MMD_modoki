...
2026-09-13、所有者は宝石サンプルを、モデルの色を参照しつつ効果を重ねる方向へ変更した。固定の石の地色でモデル色を置き換える作例を既定にせず、元の材質色・模様を下地に光学効果を追加する。具体的な混合式と強度はサンプルの実装判断であり、材質API全体の混合方式を制限しない。同日の追加依頼では、悪目立ちする縞模様をなだらかなグラデーションにするよう指定した。
