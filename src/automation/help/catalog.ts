...
export const automationHelpTopics: readonly AutomationHelpTopic[] = [
    {
        id: "menu-bar", title: "メニューバーの操作対応", aliases: ["menu", "メニュー", "設定", "表示", "背景", "全モーション削除", "描画順"],
        status: "partial", summary: "mmd_list_menu_itemsで公開メニュー名からtoolと引数を検索。不足していた視点・選択・描画設定にも対応。",
        body: "mmd_list_menu_itemsへtarget/queryを指定。返却routes.fixedArgumentsは部分的な雛形であり、target/expectedEditRevision/operationIdと必要な値を各tool schemaに従って補う。read-onlyで取得可能。status:partial/informationalの制限とnoteを読む。mmd_execute_menu_actionは現在のscope（context.timelineScope、nullも可）を照合し、停止中に実行。actionはfocusSelectedBones（現在の選択ボーンの中心を一度注視、角度/距離保持・共有Undo・キー自動登録なし）、cameraView/view、adjacentKey/direction(-1|1)、selectAllKeys/category(camera|light|shadow|gravity|bone|morph)、clearModelMotion/modelInstanceId/dryRun、moveModelRenderOrder/modelInstanceId/direction、resetSky。視点は未登録preview。前後移動はGUIの選択トラックを使う。カテゴリ全選択は100キー以上も可だが、既存コピー/編集の上限は100のまま。全モーション削除は既定dryRun:true、falseでモデルの全source animation・取込履歴・外部親キーを削除し共有UndoのeditIdを返す。モデル本体は残る。削除でsceneGenerationが変わるためcontextを再取得する。Undo/Redoは同じモデルを選択し、期待状態と履歴の先頭を照合する。描画順は結果modelsのmodelInstanceId/renderOrderを確認。設定はmmd_list_controls/mmd_set_controlのviewport.backgroundMode、viewport.skyStyle、runtime.fpsLimit、mirror.*、render.modelOrderMode/render.coplanarCorrection、physics.*、shadow.*を使用。描画順方式はモデル未読込時だけ。newProjectWindowはmmd_start_ui_operationへ指定し、完了後も新しいウィンドウのMCPはOFF。MCP権限・外部WGSL有効化、runtime/backend再起動切替はユーザー操作。",
    },
    {
    ...
    },
    {
        ...
        body: "設定→実験設定→AI連携。mmd_get_contextでtargetとeditRevisionを取得し、各toolへ渡します。書込はoperationId(UUID)とexpectedEditRevisionが必要。再生・停止・seek、カメラ・単一ボーン・モーフpreview、現在値のキー登録、一括キー編集、対象切替、設定、材質表示、AI編集Undo/Redoに対応。モデル本体・テクスチャ・頂点等の形状データは返しません。UI全項目の対応は進行中。ui-coverageで対応状況、keyframesで値の単位、timeline-transformsでミラー・補正・列編集を確認してください。手動入力やフレーム移動後はcontextを再取得してください。",
    },
    {
