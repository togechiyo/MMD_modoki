...
export const automationHelpTopics: readonly AutomationHelpTopic[] = [
    ...
    },
    {
        ...
        body: "mmd_start_ui_operationを使用し、詳細はui-operationsを参照。path、形式、対象と上書き条件を指定し、mmd_get_operationで完了を確認します。返却は結果とpathに限定し、モデル本体を送信しない条件を維持します。mmd_list_assetsは元pathの参照。WebM動画・取消・進捗と素材指定削除はvideo-and-removalを参照。PNG連番/別プロセスPNG出力、統合済みモデルモーションの個別削除/素材差替えは後続です。",
    },
    {
