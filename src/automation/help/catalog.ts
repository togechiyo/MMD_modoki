...
export const automationHelpTopics: readonly AutomationHelpTopic[] = [
    ...
    },
    {
        ...
        body: "mmd_inspectのkind:bonesまたはmorphsとmodelInstanceIdで名前と現在値を参照。mmd_set_boneへmodelInstanceId、boneName、position、rotationを渡します。rotationは度。mmd_set_morphは選択中モデルのmodelInstanceId、morphName、weight(0..1)を指定。同名の対象は拒否します。両toolでmode:preview、playbackPolicy:pauseまたはrejectが必要。キーは自動登録せず、mmd_register_keyframesで現在値を登録できます。rawキーのbone rotationsはquaternionなのでpreviewの度と混同しないこと。一括ポーズpreviewはpose-editingを参照。",
    },
    {
