...
// Opt-in diagnostic: collect observations, not a claim that Issue #24 is fixed.
...
test("Issue 24: collect AA viewport PNG and 30/60 fps WebM", async () => {
  ...
  try {
    ...
    for (const aa of [true, false]) {
      ...
      observations.push({ aa, size, backend: await page.evaluate(() => window.mmdModokiE2e.getFrameGraphPostEffectsState()) });
    }
