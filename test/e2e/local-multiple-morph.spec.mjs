...
test("local authorized model: two vertex morph visual audit", async () => {
    ...
    const output = resolve(root, "local-references/multiple-morph-audit-2026-09-17", [pbr ? "pbr" : "mmd", cpuFallback && "cpu-fallback", wasm && "wasm", skin && "skin", "reserved-capacity"].filter(Boolean).join("-"));
    ...
    try {
        ...
        for (const state of Object.values(report.states)) {
            ...
            for (const mesh of state.meshes) {
                if (mesh.usesTexture) {
                    expect(mesh.numMaxInfluencers, mesh.name).toBeGreaterThanOrEqual(8);
