...
test("shared vignette and edge blur keep independent keys and one prepared composite", async ({}, testInfo) => {
  ...
  try {
    ...
    for (const [id, position, onFrames] of [["vignette", 100, [0, 20]], ["edgeBlur", 300, [0, 40]]]) {
      await selectEffect(page, id);
      for (const frame of [0, 20, 40, 60]) {
        await seek(page, frame);
        await page.locator("#effect-key-enabled").setChecked(onFrames.includes(frame));
        await page.locator("#effect-key-value").fill(String(position));
        await page.locator("#effect-key-value").dispatchEvent("input");
    ...
    const capture = async name => {
      const output = testInfo.outputPath(name); mkdirSync(output, { recursive: true });
    ...
    for (const frame of [0, 20, 40, 60]) {
      await seek(page, frame);
      await expect(page.locator("#effect-key-enabled")).toBeChecked({ checked: frame === 0 || frame === 40 });
      pngs.push(await capture("shared-" + frame));
    }
    const diff = (a, b) => {
      let sum = 0;
    ...
    expect(diff(pngs[0], pngs[1])).toBeGreaterThan(0.001);
    expect(diff(pngs[2], pngs[3])).toBeGreaterThan(0.05);
    expect(diff(pngs[1], pngs[3])).toBeGreaterThan(1);
    await seek(page, 40);
    await selectEffect(page, "vignette");
    const remove = page.locator('[data-effect-stack-remove="vignette"]');
    ...
    await page.waitForFunction(() => window.mmdModokiE2e.getFrameGraphPostEffectsState().ready);
    expect(diff(await capture("without-vignette"), pngs[2])).toBeLessThan(0.05);
    for (const locale of ["en", "zh-Hant", "zh-Hans", "ko", "ja"]) {
      await page.locator("#toolbar-locale-select").selectOption(locale);
      const dictionary = JSON.parse(readFileSync(resolve(repoRoot, "language/" + locale + ".json"), "utf8").replace(/^\uFEFF/, ""));
      for (const { id } of cases) {
        await selectEffect(page, id);
        await expect(page.locator("[data-effect-key-label]")).toHaveText(dictionary["effect.frameGraphPost.effects." + id]);
      }
    }
    const tracks = await page.evaluate(() => window.mmdModokiE2e.exportProjectState().keyframes.effectAnimations.tracks);
    for (const id of ["vignette", "edgeBlur"]) expect(tracks.find(track => track.effectId === id).keys).toHaveLength(4);
...
const cases = [
  { id: "vignette", field: "weight", panel: "vignetteWeight", max: 4, position: 100, static: "vignetteWeight" },
  { id: "sharpen", field: "edge", panel: "sharpenEdge", max: 4, position: 100, static: "sharpenEdge" },
  { id: "chromatic", field: "amount", panel: "chromaticAberration", max: 200, position: 100, static: "chromaticAberration" },
  { id: "edgeBlur", field: "strength", panel: "edgeBlur", max: 3, position: 300, static: "dofLensEdgeBlur" },
  { id: "distortion", field: "influence", panel: "distortion", max: 1, position: 100, static: "dofLensDistortionInfluence" },
];
...
test("scalar " + config.id + " keys: " + backend + " GUI, roundtrip and output", async ({}, testInfo) => {
  ...
  try {
    ...
    await page.evaluate(path => window.mmdModokiE2e.loadModel(path), modelPath);
    await page.locator("#btn-toolbar-mode-toggle").click();
    // Wide FoV makes the existing FoV-linked distortion observable.
    const fov = page.locator("#bone-controls input[data-control-key='camFov']");
    await fov.fill("75"); await fov.press("Enter");
    ...
    if (effectId === "sharpen" || effectId === "edgeBlur") {
      // Compression removes high frequencies too. Compare transitions within the video,
      // where OFF and neutral must agree more closely than ON and OFF.
      const videoDifference = { onOff: difference(videoFrames[0], videoFrames[1]), offNeutral: difference(videoFrames[1], videoFrames[2]) };
      writeFileSync(testInfo.outputPath("video-transition-comparison.json"), JSON.stringify(videoDifference));
      expect(videoDifference.onOff).toBeGreaterThan(videoDifference.offNeutral * 2);
      expect(videoDifference.onOff).toBeGreaterThan(0.1);
      if (effectId === "edgeBlur") expect(report[0].matchingPngDifference).toBeLessThan(report[0].oppositePngDifference);
      if (effectId === "sharpen") {
        for (const item of report.slice(1)) expect(item.matchingPngDifference).toBeLessThan(item.oppositePngDifference);
      }
    } else {
