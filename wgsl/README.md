...
| サンプル | 内容 |
| --- | --- |
| [Aurora Opal](./aurora-opal.wgsl) | 元の色に、時間で流れる虹色の鉱石模様と発光する帯・縁取りを重ねる。 |
| [Moonstone Schiller](./moonstone-schiller.wgsl) | 元の色に、角度で浮かぶ柔らかな青いシラーを重ねる。 |
| [Black Opal](./black-opal.wgsl) | 元の色を残し、角度に応じて赤・緑・青に光るかけらを重ねる遊色風。 |
...
宝石4種はモデルのテクスチャ・材質色・陰影を含む`input.color`を下地にして、`input.color + effect * Coating`で光学効果だけを加算します。固定の`BodyColor`は撤去しました。Coating=1でも元の色を残し、0なら元の描画色へ戻ります。Black Opalも黒を強制しないため、黒い宝石にしたい場合はモデル側を暗い色にしてください。
