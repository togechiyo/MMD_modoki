// White Opal
// モデルの色・模様・陰影を下地に、淡い遊色を加算する。Black Opalと同じ模様の明るい合成例。

// ---- ここを編集: 見た目の調整 ----
// かけらの細かさ。目安0.1〜16。物体座標への倍率。大きいほど細かい。
const FLAKE_SCALE: f32 = 5.0;
// 遊色の強さ。目安0〜2。0で遊色なし。白い表面光沢は残る。
const COLOR_STRENGTH: f32 = 1.1;
...
fn effectFinalColor(input: ModokiFinalColor) -> vec3f {
    ...
    let domain = whiteOpalDomain(local * FLAKE_SCALE * vec3f(1.0, 1.45, 1.0));
    let normal = whiteOpalUnit(s.normalWS);
    let view = whiteOpalUnit(effectInputs.CAMERA_POSITION - s.positionWS);
    let light = whiteOpalUnit(-effectInputs.LIGHT_DIRECTION);
    let halfVector = whiteOpalUnit(view + light);
    let axis = whiteOpalUnit((transpose(effectInputs.WORLDINVERSE) * vec4f(domain.xyz * 2.0 - vec3f(1.0), 0.0)).xyz);
    ...
    let colour = saturated * flake * flash * COLOR_STRENGTH * 0.55 * illumination;
    ...
    return input.color + (colour + vec3f(gloss + rim)) * COATING;
}
